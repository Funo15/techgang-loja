...
// Remove encomendas pendentes com mais de 2h (cliente abandonou o Stripe).
// Nunca apaga encomendas pagas, reembolsadas ou canceladas.
function limparPendentesAntigos() {
  ...
  `).run();
  if (r.changes > 0) {
    console.log(`[manutencao] ${r.changes} encomenda(s) pendente(s) antigas removidas`);
  }
  return r.changes;
}

module.exports = { fazerBackup, limparBackupsAntigos, limparPendentesAntigos, agendarBackupDiario };
