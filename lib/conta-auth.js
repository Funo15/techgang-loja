...
async function autenticar(email, password) {
  ...
  if (!c) return null;
  // Contas só-Google têm password_hash/salt vazios — não permitem login por password.
  if (!c.password_hash || !c.salt) return null;
  const hash = await hashPassword(password, c.salt);
  // Defesa contra RangeError do timingSafeEqual: garantir mesmo comprimento.
  const a = Buffer.from(hash);
  const b = Buffer.from(c.password_hash);
...
async function autenticarOuRegistarGoogle(credential, clientId) {
  ...
  const ISSUERS_VALIDOS = ['accounts.google.com', 'https://accounts.google.com'];
  if (!ISSUERS_VALIDOS.includes(p.iss)) throw new Error('Emissor não autorizado.');
  ...
  // 1. Conta já ligada ao Google → login direto
  let c = db.prepare('SELECT * FROM customers WHERE google_id = ?').get(googleId);
  if (c) return c;

  // 2. Existe conta com este email mas SEM google_id → foi criada com password.
  //    Não fazer merge automático: um atacante podia pré-registar o email da vítima
  //    e ganhar acesso quando ela entrasse via Google.
  const existente = db.prepare('SELECT id, google_id FROM customers WHERE email = ?').get(email);
  if (existente) {
    if (!existente.google_id) {
      throw new Error('Já tens uma conta com este email. Entra com a tua password para continuar.');
    }
    // google_id existe mas não coincide com o token atual — conta associada a outro Google
    throw new Error('Esta conta já está associada a outro acesso Google.');
  }

  // 3. Email novo → criar conta Google
  db.prepare('INSERT INTO customers (nome, email, password_hash, salt, google_id) VALUES (?, ?, ?, ?, ?)')
    .run(nome, email, '', '', googleId);
  return db.prepare('SELECT * FROM customers WHERE google_id = ?').get(googleId);
}
