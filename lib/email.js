  ...
  const linhas = items.map(i => `
    <tr>
      <td style="padding:8px 0;border-bottom:1px solid #E8E7EE;font-size:14px;">
        ${i.nome}${i.cor ? ` <span style="color:#8A8793;">· ${i.cor}</span>` : ''} × ${i.qtd}
      </td>
...
async function enviarEmailsEncomenda(order) {
  ...
  try {
    await transporter.sendMail({
      ...
      html: layoutEmail(`Obrigado, ${order.nome_cliente.split(' ')[0]}!`, `
  ...
  try {
    ...
    const linhasFornecedor = items.map(i => {
      ...
      return `<li style="font-size:13px;margin-bottom:6px;">
        ${i.nome}${i.cor ? ` (${i.cor})` : ''} × ${i.qtd} —
        ${p?.fornecedor || '?'} · ref ${p?.fornecedor_product_id || '?'} ·
        <a href="${p?.fornecedor_url || '#'}" style="color:${config.cores.acento};">abrir no fornecedor</a>
      </li>`;
    }).join('');
    ...
    await transporter.sendMail({
      ...
      html: layoutEmail(`Nova encomenda paga: ${order.numero_encomenda}`, `
        ...
        <p style="font-size:13px;line-height:1.7;margin:0;">
          ...
          ${order.morada}<br>
          ${order.codigo_postal} ${order.cidade}, ${order.pais}<br>
          ${order.email}${order.telefone ? ` · ${order.telefone}` : ''}
        </p>`)
    });
...
async function enviarEmailTracking(order) {
  ...
  try {
    ...
    await criarTransporter().sendMail({
      ...
      html: layoutEmail(`Já vai a caminho, ${order.nome_cliente.split(' ')[0]}!`, `
        <p style="font-size:14px;line-height:1.6;margin:0 0 14px;">
